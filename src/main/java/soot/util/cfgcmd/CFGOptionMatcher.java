...
/**
  ...
  public CFGOption match(String quarry) {
    ...
    for (int i = 0; i < options.length; i++) {
      ...
      if (uncasedName.startsWith(uncasedQuarry)) {
        if (match == -1) {
          ...
          logger.debug(new StringBuilder().append("").append(quarry).append(" is ambiguous; it matches ").append(options[match].name()).append(" and ").append(options[i].name())
				.toString());
    ...
    if (match == -1) {
      logger.debug(new StringBuilder().append("\"").append(quarry).append("\"").append(" does not match any value.").toString());
      throw new CompilationDeathException(CompilationDeathException.COMPILATION_ABORTED, "Option parse error");
    } else {
