...
private static PrintWriter openBodyFile(Body b, String baseName) throws java.io.IOException {
    ...
    String filePath = new StringBuilder().append(dir.toString()).append(File.separatorChar).append(baseName).toString();
...
/**
  ...
  private static String nextGraphFileName(Body b, String baseName) throws java.io.IOException {
    ...
    File dir = makeDirectoryIfMissing(b);
    final String prefix = new StringBuilder().append(dir.toString()).append(File.separatorChar).append(baseName).toString();
    File file = null;
    int fileNumber = 0;
    do {
      file = new File(new StringBuilder().append(prefix).append(fileNumber).append(DotGraph.DOT_EXTENSION).toString());
      fileNumber++;
    } while (file.exists());
...
private static void deleteOldGraphFiles(final Body b, final String phaseName) {
    try {
      ...
      final File[] toDelete = dir.listFiles((File dir1, String name) -> name.startsWith(phaseName) && name.endsWith(DotGraph.DOT_EXTENSION));
      if (toDelete != null) {
...
public void dumpBody(Body b, String baseName) {
    try (java.io.PrintWriter out = openBodyFile(b, baseName)) {
      alreadyDumping = true;
...
/**
  ...
  public void dumpAfter(Body b, String phaseName) {
    ...
    if (!poppedPhaseName.equals(phaseName)) {
      throw new IllegalArgumentException(new StringBuilder().append("dumpAfter(").append(phaseName).append(") when poppedPhaseName == ").append(poppedPhaseName).toString());
    }
    if (isBodyDumpingPhase(phaseName)) {
  ...
  public void dumpGraph(DirectedGraph g, Body b) {
    ...
    try {
      ...
      if (isCFGDumpingPhase(phaseName)) {
        try {
          String outputFile = nextGraphFileName(b, new StringBuilder().append(phaseName).append("-").append(getClassIdent(g)).append("-").toString());
...
/**
  ...
  public void dumpGraph(ExceptionalGraph g) {
    ...
    try {
      ...
      if (isCFGDumpingPhase(phaseName)) {
        try {
          String outputFile = nextGraphFileName(g.getBody(), new StringBuilder().append(phaseName).append("-").append(getClassIdent(g)).append("-").toString());
