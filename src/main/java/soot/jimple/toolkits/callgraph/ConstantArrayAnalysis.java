...
public class ConstantArrayAnalysis extends ForwardFlowAnalysis<Unit, ConstantArrayAnalysis.ArrayState> {
  private Map<Local, Integer> localToInt = new HashMap<>();
	...
	public ConstantArrayAnalysis(DirectedGraph<Unit> graph, Body b) {
	    super(graph);
	    b.getLocals().forEach(l -> localToInt.put(l, size++));
	...
	@Override
	  protected void flowThrough(ArrayState in, Unit d, ArrayState out) {
	    ...
	    if (d instanceof DefinitionStmt) {
	      ...
	      rhs.getUseBoxes().stream().filter(b -> localToInt.containsKey(b.getValue())).mapToInt(b -> localToInt.get(b.getValue())).forEach(localRef -> {
			out.state[localRef] = null;
			out.active.set(localRef);
		});
	      ...
	      d.getUseBoxes().stream().filter(b -> localToInt.containsKey(b.getValue())).mapToInt(b -> localToInt.get(b.getValue())).forEach(localRef -> {
			out.state[localRef] = null;
			out.active.set(localRef);
		});
	    }
...
private class ArrayTypesInternal implements Cloneable {
    private final Logger logger = LoggerFactory.getLogger(ArrayTypesInternal.class);
	BitSet mustAssign;
    BitSet typeState[];
  ...
  public static class ArrayTypes {
    ...
    public String toString() {
      return new StringBuilder().append("ArrayTypes [possibleSizes=").append(possibleSizes).append(", possibleTypes=").append(Arrays.toString(possibleTypes)).append("]").toString();
    }
  }
