...
public boolean isObjectLocalToContext(Value localOrRef, SootMethod sm, SootMethod context) {
    ...
    if (localOrRef instanceof StaticFieldRef) {
      if (dfa.printDebug()) {
        logger.debug(new StringBuilder().append("    SHARED (Static             from ").append(context.getDeclaringClass().getShortName()).append(".").append(context.getName()).append(")").toString());
      }
      return false;
    }
    ...
    if (mergedContext == null) {
      if (dfa.printDebug()) {
        logger.debug(new StringBuilder().append("      ------ (Unreachable        from ").append(context.getDeclaringClass().getShortName()).append(".").append(context.getName()).append(")").toString());
      }
    ...
    if (localOrRef instanceof InstanceFieldRef) {
      ...
      if (ifr.getBase() == thisLocal) {
        ...
        if (dfa.printDebug()) {
          if (isLocal) {
            logger.debug(new StringBuilder().append("      LOCAL  (this  .localField  from ").append(context.getDeclaringClass().getShortName()).append(".").append(context.getName()).append(")").toString());
          } else {
            logger.debug(new StringBuilder().append("      SHARED (this  .sharedField from ").append(context.getDeclaringClass().getShortName()).append(".").append(context.getName()).append(")").toString());
          }
        }
        return isLocal;
      } else {
        ...
        if (isLocal) {
          ...
          if (dfa.printDebug()) {
            if (isLocal) {
              logger.debug(new StringBuilder().append("      LOCAL  (local .localField  from ").append(context.getDeclaringClass().getShortName()).append(".").append(context.getName()).append(")").toString());
            } else {
              logger.debug(new StringBuilder().append("      SHARED (local .sharedField from ").append(context.getDeclaringClass().getShortName()).append(".").append(context.getName()).append(")").toString());
            }
          }
          return isLocal;
        } else {
          if (dfa.printDebug()) {
            logger.debug(new StringBuilder().append("      SHARED (shared.someField   from ").append(context.getDeclaringClass().getShortName()).append(".").append(context.getName()).append(")").toString());
          }
          return isLocal;
        }
      }
    ...
    if (dfa.printDebug()) {
      if (isLocal) {
        logger.debug(new StringBuilder().append("      LOCAL  ( local             from ").append(context.getDeclaringClass().getShortName()).append(".").append(context.getName()).append(")").toString());
      } else {
        logger.debug(new StringBuilder().append("      SHARED (shared             from ").append(context.getDeclaringClass().getShortName()).append(".").append(context.getName()).append(")").toString());
      }
    }
    return isLocal;
  }
    ...
    while (edgeIt.hasNext()) {
      ...
      if (!path.containsMethod(node) && e.isExplicit() && e.srcStmt().containsInvokeExpr()) {
        ...
        CallChain newpath = getNextCallChainBetween(rm, start, node, e, path, previouslyFound); // node is supposed to be a
                                                                                                boolean condition = newpath != null && !previouslyFound.contains(newpath);
		// logger.debug("|");
		// }
      ...
      while (graphIt.hasNext()) {
        ...
        if (node instanceof FieldRef) {
          ...
          if (dataFlowGraph.getPredsOf(nodeEqVal).size() > 0 || dataFlowGraph.getSuccsOf(nodeEqVal).size() > 0) {
            ...
            boolean condition = pr.getIndex() != -1 && !isObjectLocalToContext(ie.getArg(pr.getIndex()), containingMethod, context);
			if (condition) {
                return true;
              }
          }
      ...
      if (isObjectLocalToContext(iie.getBase(), containingMethod, context)) {
        ...
        while (graphIt.hasNext()) {
          ...
          if (node instanceof StaticFieldRef) {
            ...
            if (dataFlowGraph.getPredsOf(nodeEqVal).size() > 0 || dataFlowGraph.getSuccsOf(nodeEqVal).size() > 0) {
              ParameterRef pr = (ParameterRef) node;
              boolean condition1 = pr.getIndex() != -1 && !isObjectLocalToContext(ie.getArg(pr.getIndex()), containingMethod, context);
			if (condition1) {
			  return true;
			}
            }
      ...
      // For a instance invoke on shared object, check if any fields or any shared params are read/written
        ...
        while (graphIt.hasNext()) {
          ...
          if (node instanceof FieldRef) {
            ...
            if (dataFlowGraph.getPredsOf(nodeEqVal).size() > 0 || dataFlowGraph.getSuccsOf(nodeEqVal).size() > 0) {
              ParameterRef pr = (ParameterRef) node;
              boolean condition2 = pr.getIndex() != -1 && !isObjectLocalToContext(ie.getArg(pr.getIndex()), containingMethod, context);
			if (condition2) {
			  return true;
			}
            }
