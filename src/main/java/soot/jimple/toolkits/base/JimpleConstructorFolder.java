    ...
    for (Unit u : stmtList) {
      ...
      for (ValueBox usebox : s.getUseBoxes()) {
        ...
        if (before.get(local) != null) {
          throw new RuntimeException(new StringBuilder().append("Use of an unitialized value ").append("before constructor call; are you sure this ").append("bytecode is verifiable?\n").append(s).toString());
        }
      }
    }

    // throw out all new statements
	stmtList.stream().map(u -> (Stmt) u).forEach(s -> {
		if (isNew(s)) {
		    units.remove(s);
		  }
	});
...
private class Fact {
    ...
    public void removeAll(Stmt s) {
      stmtToVar.get(s).forEach(varToStmt::remove);
    ...
    public void mergeFrom(Fact in1, Fact in2) {
      ...
      in2.varToStmt.entrySet().forEach(e -> {
        Local l = e.getKey();
        Stmt newStmt = e.getValue();
        add(l, newStmt);
      });
