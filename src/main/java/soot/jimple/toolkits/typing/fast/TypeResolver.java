...
public class TypeResolver {
  ...
  public TypeResolver(JimpleBody jb) {
    ...
    this.jb.getLocals().forEach(this::addLocal);
    this.initAssignments();
  }

  private void initAssignments() {
    this.jb.getUnits().stream().filter(stmt -> stmt instanceof DefinitionStmt).forEach(stmt -> this.initAssignment((DefinitionStmt) stmt));
  }
