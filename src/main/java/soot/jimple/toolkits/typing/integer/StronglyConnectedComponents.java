...
private StronglyConnectedComponents(List<TypeVariable> typeVariableList) throws TypeException {
    ...
    variables.stream().filter(var -> !black.contains(var)).forEach(var -> {
        black.add(var);
        dfsg_visit(var);
      });
...
private void dfsg_visit(TypeVariable var) {
    ...
    parents.stream().filter(parent -> !black.contains(parent)).forEach(parent -> {
        black.add(parent);
        dfsg_visit(parent);
      });
...
private void dfsgt_visit(TypeVariable var) {
    ...
    children.stream().filter(child -> !black.contains(child)).forEach(child -> {
        black.add(child);
        dfsgt_visit(child);
      });
  }
