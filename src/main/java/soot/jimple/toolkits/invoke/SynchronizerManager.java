	...
	/**
	  ...
	  public SootMethod getClassFetcherFor(SootClass c) {
	    ...
	    for (; true; methodName = "_" + methodName) {
	      ...
	      // Check signature.
	      if (!m.getSignature()
	          .equals(new StringBuilder().append("<").append(c.getName().replace('.', '$')).append(": java.lang.Class ").append(methodName).append("(java.lang.String)>").toString())) {
	        continue;
	      }
		...
		Value ie = as.getRightOp();
	      ...
	      if (!ie.toString().equals(new StringBuilder().append(".staticinvoke <java.lang.Class: java.lang.Class forName(java.lang.String)>(").append(lo).append(")").toString())) {
	        continue;
	      }
