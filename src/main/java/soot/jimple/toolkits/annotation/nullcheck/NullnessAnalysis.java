  ...
  protected static final int BOTTOM = 0;
	...
	private void handleIfStmt(JIfStmt ifStmt, AnalysisInfo in, AnalysisInfo out, AnalysisInfo outBranch) {
	    ...
	    if (condition instanceof JInstanceOfExpr) {
	      ...
	      handleInstanceOfExpression(expr, outBranch);
	    } else if (condition instanceof JEqExpr || condition instanceof JNeExpr) {
	      // a==b or a!=b
	      AbstractBinopExpr eqExpr = (AbstractBinopExpr) condition;
	      handleEqualityOrNonEqualityCheck(eqExpr, out, outBranch);
	    }
	  }

	private void handleEqualityOrNonEqualityCheck(AbstractBinopExpr eqExpr, AnalysisInfo out,
	...
	private void handleInstanceOfExpression(JInstanceOfExpr expr, AnalysisInfo outBranch) {
	    Value op = expr.getOp();
