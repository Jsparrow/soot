...
class RegisterAssigner {
  ...
  private RegisterAllocator regAlloc;
	...
	private void renumParamRegsToHigh(List<Insn> insns, List<LocalRegisterAssignmentInformation> parameterInstructionsList) {
	    ...
	    insns.forEach(insn -> insn.getRegs().forEach(r -> renumParamRegToHigh(r, regCount, paramRegCount)));
	    parameterInstructionsList.forEach(parameter -> renumParamRegToHigh(parameter.getRegister(), regCount, paramRegCount));
	  }
	...
	/**
	  ...
	  private void reserveRegisters(List<Insn> insns, Map<Insn, Stmt> insnsStmtMap,
		...
		insns.forEach(insn -> shiftRegs(insn, regsToReserve));
	      parameterInstructionsList.stream().map(LocalRegisterAssignmentInformation::getRegister).forEach(r -> r.setNumber(r.getNumber() + regsToReserve));
	...
	private void shiftRegs(Insn insn, int shiftAmount) {
	    insn.getRegs().forEach(r -> r.setNumber(r.getNumber() + shiftAmount));
	  }
	...
	private void fixIncompatRegs(Insn insn, InstructionIterator allInsns) {
	    ...
	    if (hasResultReg && isResultRegIncompat && !insn.getOpcode().name.endsWith("/2addr")
	        && !"check-cast".equals(insn.getOpcode().name)) {
	...
	private Insn findFittingInsn(Insn insn) {
	    ...
	    if (insn instanceof Insn11n && opc == Opcode.CONST_4) {
