...
public class StmtVisitor implements StmtSwitch {
  ...
  private static final Opcode[] OPCODES = Opcode.values();
	...
	public void addInsn(Insn insn, Stmt s) {
	    insns.add(insn);
	    boolean condition = s != null && insnStmtMap.put(insn, s) != null;
		if (condition) {
	...
	private boolean isJumpTarget(Stmt target) {
	    for (Insn insn : this.insns) {
	      boolean condition = insn instanceof InsnWithOffset && ((InsnWithOffset) insn).getTarget() == target;
		if (condition) {
		  return true;
		}
	    }
	...
	@Override
	  public void defaultCase(Object o) {
	    ...
	    throw new Error(new StringBuilder().append("unknown Object (").append(o.getClass()).append(") as Stmt: ").append(o).toString());
	  }
	...
	private Insn buildMonitorInsn(MonitorStmt stmt, Opcode opc) {
	    ...
	    boolean condition = lockValue instanceof Constant && (lockReg = monitorRegs.get(lockValue)) != null;
		if (condition) {
	...
	private Opcode getPutGetOpcodeWithTypeSuffix(int opcodeBase, String fieldType) {
	    if ("Z".equals(fieldType)) {
	      return OPCODES[opcodeBase + BOOLEAN_OFFSET];
	    } else if ("I".equals(fieldType) || "F".equals(fieldType)) {
