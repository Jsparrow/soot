...
public class SynchronizedBlockFinder implements FactFinder {
  private HashMap<AugmentedStmt, Map<Value, Integer>> as2ml;
	...
	private final Integer white = Integer.valueOf(0);// never visited in DFS
	private final Integer gray = Integer.valueOf(1);// visited but not finished
	private final Integer black = Integer.valueOf(2);// finished
	private final int unknown = -100000; // Note there are at most 65536 monitor
	// exits in a method.
	  private final Integer variableIncr = Integer.valueOf(unknown);
	    ...
	    while (asgit.hasNext()) {
	      ...
	      if (as.get_Stmt() instanceof EnterMonitorStmt) {
	        ...
	        if (synchSet != null) {
	          ...
	          if (!done) {
	            throw new RuntimeException(new StringBuilder().append("Could not verify approximated Synchronized body!\n").append("Method:\n").append(body.getMethod()).append("Body:\n").append("===============================================================\n").append(body.getUnits())
						.append("===============================================================\n").toString());
	          }
	  ...
	  private void find_VariableIncreasing(AugmentedStmtGraph asg, HashMap local2level_template,
	    ...
	    while (worklist.isEmpty() == false) {
	      ...
	      while (sit.hasNext()) {
	        ...
	        while (mlsit.hasNext()) {
	          ...
	          // worklist
	          if ((local2level.get(local) == variableIncr) && (!slocal2level.get(local).equals(variableIncr))) {
	            slocal2level.put(local, variableIncr);
	    ...
	    while (asgit.hasNext()) {
	      ...
	      while (mlsit.hasNext()) {
	        ...
	        if (local2level.get(local) == variableIncr) {
	  ...
	  private void DFS_Scc(AugmentedStmt as, IterableSet component, HashMap as2rml, HashMap as2color, AugmentedStmt seedStmt,
	    ...
	    if (s instanceof MonitorStmt) {
	      ...
	      if (s instanceof EnterMonitorStmt) {
	        ...
	        local2level.put(local, Integer.valueOf(local2level.get(local).intValue() + 1));
	      } else {
	...
	/*
	  ...
	  private void set_MonitorLevels(AugmentedStmtGraph asg) {
	    ...
	    while (mlsit.hasNext()) {
	      ...
	      local2level_template.put(mlsit.next(), Integer.valueOf(0));
	    }
	    ...
	    viAugStmts.forEach(vias -> {
	      ...
	      while (lit.hasNext()) {
	        ...
	        local2level.put(lit.next(), variableIncr);
	      }
	    });
	      ...
	      while (pit.hasNext()) {
	        ...
	        while (mlsit.hasNext()) {
	          ...
	          if (predLevel > curLevel) {
	            cur_local2level.put(local, Integer.valueOf(predLevel));
	...
	private boolean verify_CatchBody(ExceptionNode en, IterableSet synchBody, Value monitorVariable, Value copiedLocal) {
	    ...
	    while (as.get_Stmt() instanceof GotoStmt) {
	      ...
	      if (entryPointTarget != null) {
	        boolean condition = (as.get_Stmt() != entryPointTarget) && (as.cpreds.size() != 1) && as.cpreds.size() != 1;
			if (condition) {
	    ...
	    boolean condition1 = (((ExitMonitorStmt) s).getOp() != monitorVariable) && (((ExitMonitorStmt) s).getOp() != copiedLocal);
		if (condition1) {
