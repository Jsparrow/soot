  ...
  public static String dependancies[] = new String[] { "bb.jbco_rlaii", "bb.jbco_ful", "bb.lp" };
	...
	protected void internalTransform(Body b, String phaseName, Map<String, String> options) {
	    ...
	    while (worklist.size() > 0) {
	      ...
	      if (u instanceof TargetArgInst && checkCandidate(succs, bug)) {
		  candidates.add(u);
		}
	
	      worklist.addAll(succs.stream().filter(o -> !visited.contains(o)).collect(Collectors.toList()));
	    }
		...
		if (output) {
	        out.println(new StringBuilder().append(movedloads - orig).append(" loads moved above ifs in ").append(b.getMethod().getSignature()).toString());
	      }
		if (debug) {
