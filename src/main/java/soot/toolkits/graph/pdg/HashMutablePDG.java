...
public class HashMutablePDG extends HashMutableEdgeLabelledDirectedGraph<PDGNode, String> implements ProgramDependenceGraph {
  ...
  private static Hashtable<PDGNode, PDGRegion> node2Region = new Hashtable<>();
	...
	private RegionAnalysis mRegionAnalysis = null;
	private int mStrongRegionStartId;
	...
	public HashMutablePDG(UnitGraph cfg) {
	    ...
	    this.mRegionAnalysis = new RegionAnalysis(this.m_cfg, this.m_body.getMethod(), this.m_class);
	
	    /*
	     * Get the weak regions and save a copy. Note that the strong regions list is initially cloned from the weak region to be
	     * later modified.
	     */
	    this.m_strongRegions = this.mRegionAnalysis.getRegions();
	    this.m_weakRegions = this.cloneRegions(this.m_strongRegions);
	    this.m_blockCFG = this.mRegionAnalysis.getBlockCFG();
	...
	/**
	  ...
	  protected void constructPDG() {
	    Hashtable<Block, Region> block2region = this.mRegionAnalysis.getBlock2RegionMap();
	    DominatorTree<Block> pdom = this.mRegionAnalysis.getPostDominatorTree();
	    DominatorTree<Block> dom = this.mRegionAnalysis.getDominatorTree();
	
	    List<Region> regions2process = new LinkedList<>();
	    Region topLevelRegion = this.mRegionAnalysis.getTopLevelRegion();
	    mStrongRegionStartId = m_weakRegions.size();
	          ...
	          while (!copyOfDependents.isEmpty()) {
	            ...
	            if (dependents.containsAll(rdepB.getBlocks())) {
	              ...
	              if (predPDGofdepB == pdgnodeOfdepBRegion) {
	                ...
	                Region newRegion = new Region(this.mStrongRegionStartId++, topLevelRegion.getSootMethod(),
	...
	private List<Region> cloneRegions(List<Region> weak) {
	    ...
	    weak.forEach(r -> strong.add((Region) r.clone()));
	    return strong;
	  }
	...
	private static PDGRegion pdgpostorder(PDGNode node, List<PDGRegion> list) {
	    ...
	    if (!dependents.isEmpty()) {
	      for (PDGNode dependent : dependents) {
	        PDGNode curNode = (PDGNode) dependent;
	        ...
	        if (curNode instanceof LoopedPDGNode) {
	          ...
	          for (PDGNode child1 : childs) {
	            PDGNode child = (PDGNode) child1;
	...
	public List<PDGNode> getDependents(PDGNode node) {
	    ...
	    toReturn.addAll(this.getSuccsOf(node).stream().filter(succ -> this.dependentOn(succ, node)).collect(Collectors.toList()));
	    return toReturn;
	  }
	...
	public PDGNode getPDGNode(Object cfgNode) {
	    if (cfgNode instanceof Block && this.m_obj2pdgNode.containsKey(cfgNode)) {
	...
	@SuppressWarnings("unchecked")
	  private void replaceInGraph(PDGNode newnode, PDGNode oldnode) {
	    ...
	    succs.forEach(succ -> {
	      ...
	      labels.forEach(label -> this.addEdge(newnode, succ, label.toString()));
	    });
	
	    preds.forEach(pred -> {
	      List<Object> labels = graph.getLabelsForEdges(pred, oldnode);
	      labels.forEach(label -> this.addEdge(pred, newnode, label.toString()));
	    });
	...
	public void removeAllEdges(PDGNode from, PDGNode to) {
	    ...
	    labels.forEach(label -> this.removeEdge(from, to, label));
	  }
	  ...
	  public String toString() {
	    ...
	    while (nodes.peek() != null) {
	      ...
	      for (PDGNode succ : succs) {
	        ...
	        s += i++;
	        s += new StringBuilder().append(": Edge's label: ").append(labels).append(" \n").toString();
	        s += "   Target: " + succ.toShortString();
	        s += "\n";
	
	        boolean condition = "dependency".equals(labels.get(0)) && !processed.contains(succ);
			if (condition) {
	            nodes.offer(succ);
	          }
