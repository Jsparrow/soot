...
public void printAstMetric(Node n, CodeWriter w) {
    if (n instanceof Stmt && stmtToMetric.containsKey(n)) {
        w.write(new StringBuilder().append(" // sum= ").append(stmtToMetric.get(n)).append(" : depth= ").append(stmtToMetricDepth.get(n)).append("\t").toString());
      }
  }
...
public void addMetrics(ClassData data) {
    ...
    data.addMetric(new MetricData("D-W-Complexity", Double.valueOf(sum)));

    data.addMetric(new MetricData("AST-Node-Count", Integer.valueOf(numNodes)));
  }
...
public NodeVisitor enter(Node parent, Node n) {
    ...
    // switch from Stmt to Expr here, since Expr is the smallest unit
      ...
      logger.info(new StringBuilder().append(sum).append("  ").append(n).append("  ").toString());
      sum += currentDepth * 2;
    ...
    if (n instanceof Stmt) {
      stmtToMetric.put(n, Integer.valueOf(sum));
      stmtToMetricDepth.put(n, Integer.valueOf(currentDepth));
    }
