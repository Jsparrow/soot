...
public CombinedDUAnalysis(UnitGraph graph) {
    ...
    if (Options.v().verbose()) {
      logger.debug(new StringBuilder().append("[").append(graph.getBody().getMethod().getName()).append("]     Constructing CombinedDUAnalysis...").toString());
    }
    ...
    for (Unit u : graph) {
      ...
      if (defs.size() == 1) {
        ...
        throw new RuntimeException(new StringBuilder().append("Locals defed in ").append(u).append(": ").append(defs.size()).toString());
      }
      ...
      for (ValueBox vb : u.getUseBoxes()) {
        ...
        if (useBoxToUnit.containsKey(vb)) {
          throw new RuntimeException(new StringBuilder().append("Aliased ValueBox ").append(vb).append(" in Unit ").append(u).toString());
        }
    ...
    if (Options.v().verbose()) {
      logger.debug(new StringBuilder().append("[").append(graph.getBody().getMethod().getName()).append("]     Finished CombinedDUAnalysis...").toString());
    }
