...
public class InitialResolver implements IInitialResolver {
  ...
  private HashMap<String, Node> sootNameToAST = null;
	...
	private void createLocalAndAnonClassNames(ArrayList<Node> anonBodyList, ArrayList<Node> localClassDeclList) {
	    anonBodyList.forEach(anAnonBodyList -> createAnonClassName((polyglot.ast.New) anAnonBodyList));
	    localClassDeclList.forEach(aLocalClassDeclList -> createLocalClassName((polyglot.ast.LocalClassDecl) aLocalClassDeclList));
	  }
	...
	private void createAnonClassName(polyglot.ast.New nextNew) {
		...
		String realName = new StringBuilder().append(outerToMatch.fullName()).append("$").append(nextAvailNum).toString();
	...
	private void createLocalClassName(polyglot.ast.LocalClassDecl lcd) {
		...
		String realName = new StringBuilder().append(outerToMatch.fullName()).append("$").append(nextAvailNum).append(lcd.decl().name()).toString();
	...
	private int getAnonClassNum(String realName) {
	    ...
	    return (Integer.valueOf(realName.substring(dIndex + 1))).intValue();
	  }
	...
	private void buildFinalLocalMap(ArrayList<Node> memberList) {
	    memberList.forEach(aMemberList -> handleFinalLocals((polyglot.ast.ClassMember) aMemberList));
	  }
	...
	public boolean isAnonInCCall(polyglot.types.ClassType anonType) {
	    for (Node aCCallList : cCallList) {
	      polyglot.ast.ConstructorCall cCall = (polyglot.ast.ConstructorCall) aCCallList;
	      // System.out.println("cCall params: "+cCall.arguments());
	      Iterator argsIt = cCall.arguments().iterator();
	      while (argsIt.hasNext()) {
	        Object next = argsIt.next();
	        boolean condition = next instanceof polyglot.ast.New && ((polyglot.ast.New) next).anonType() != null && ((polyglot.ast.New) next).anonType().equals(anonType);
