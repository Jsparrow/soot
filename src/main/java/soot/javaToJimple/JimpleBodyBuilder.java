  ...
  protected List<List<Stmt>> beforeReturn; // list used to exclude return
	...
	/**
	  ...
	  public soot.jimple.JimpleBody createJimpleBody(polyglot.ast.Block block, List formals, soot.SootMethod sootMethod) {
	    ...
	    if ((outerIndex != -1) && ("<init>".equals(sootMethod.getName()))) {
	    ...
	    if (exceptionTable != null) {
	      exceptionTable.forEach(anExceptionTable -> body.getTraps().add(anExceptionTable));
	    }
	    return body;
	...
	protected void handleFieldInits(ArrayList<FieldDecl> fieldInits) {
	    for (FieldDecl field : fieldInits) {
	...
	/**
	   ...
	   */
	  private void handleOuterClassThisInit() {
	...
	/**
	  ...
	  private void handleStaticFieldInits(soot.SootMethod sootMethod) {
		...
		staticFieldInits.forEach(field -> {
	        String fieldName = field.name();
	        polyglot.ast.Expr initExpr = field.init();
	        soot.SootClass currentClass = body.getMethod().getDeclaringClass();
	        soot.SootFieldRef sootField = soot.Scene.v().makeFieldRef(currentClass, fieldName,
	...
	protected void handleStaticBlocks(ArrayList<Block> initializerBlocks) {
	    initializerBlocks.forEach(this::createBlock);
	  }
	...
	/**
	  ...
	  private void handleStaticInitializerBlocks(soot.SootMethod sootMethod) {
		...
		staticInitializerBlocks.forEach(this::createBlock);
	  }
	      ...
	      while (!fieldFound) {
	        if (!currentClass.declaresFieldByName("this$0")) {
	          throw new RuntimeException(
	              new StringBuilder().append("Trying to get field val$").append(li.name()).append(" from some outer class but can't access the outer class of: ").append(currentClass.getName()).append("!").append(" current class contains fields: ")
						.append(currentClass.getFields()).toString());
	        }
	...
	private boolean needSootIf(soot.Value sootCond) {
	    boolean condition = sootCond instanceof soot.jimple.IntConstant && ((soot.jimple.IntConstant) sootCond).value == 1;
		if (condition) {
	        return false;
	      }
	    return true;
	...
	/**
	  ...
	  private void createSwitch(polyglot.ast.Switch switchStmt) {
	    ...
	    if (isLookupSwitch(switchStmt)) {
	      ...
	      while (it.hasNext()) {
	        Object next = it.next();
	        boolean condition = next instanceof polyglot.ast.Case && !((polyglot.ast.Case) next).isDefault();
			if (condition) {
	...
	/**
	  ...
	  private void createReturn(polyglot.ast.Return retStmt) {
	    ...
	    if (monitorStack != null) {
	      ...
	      {
	        beforeReturn.forEach(v -> v.add(stopNoop));
	      }
	    }
	    ...
	    {
	      afterReturn.forEach(v -> v.add(startNoop));
	    }
	  }
	...
	/**
	  ...
	  protected soot.Value createAggressiveExpr(polyglot.ast.Expr expr, boolean reduceAggressively, boolean reverseCondIfNec) {
	    ...
	    if (expr.isConstant() && expr.constantValue() != null && expr.type() != null
	        && !(expr instanceof polyglot.ast.Binary && "java.lang.String".equals(expr.type().toString()))) {
	  ...
	  protected soot.Local handlePrivateFieldAssignSet(polyglot.ast.Assign assign) {
	    ...
	    if (assign.operator() == polyglot.ast.Assign.ASSIGN) {
	      right = base().getSimpleAssignRightLocal(assign);
	        ...
	        && "java.lang.String".equals(assign.type().toString())) {
	...
	@Override
	  protected soot.Value getAssignRightLocal(polyglot.ast.Assign assign, soot.Local leftLocal) {
	    ...
	    } else if (assign.operator() == polyglot.ast.Assign.ADD_ASSIGN && "java.lang.String".equals(assign.type().toString())) {
	...
	/**
	  ...
	  protected soot.Value getAssignLocal(polyglot.ast.Assign assign) {
	    ...
	    if ((assign.operator() == polyglot.ast.Assign.ADD_ASSIGN) && "java.lang.String".equals(assign.type().toString())) {
	...
	/**
	  ...
	  private soot.Value getFieldLocalLeft(polyglot.ast.Field field) {
	    ...
	    if (("length".equals(field.name())) && (receiver.type() instanceof polyglot.types.ArrayType)) {
	...
	/**
	  ...
	  private soot.Value getFieldLocal(polyglot.ast.Field field) {
	    ...
	    if (("length".equals(field.name())) && (receiver.type() instanceof polyglot.types.ArrayType)) {
	      return getSpecialArrayLengthLocal(field);
	    } else if ("class".equals(field.name())) {
	...
	private Number createConstantCast(polyglot.types.Type fieldType, Number constant) {
	    if (constant instanceof Integer) {
	      if (fieldType.isDouble()) {
	        return Double.valueOf(((Integer) constant).intValue());
	      } else if (fieldType.isFloat()) {
	        return Float.valueOf(((Integer) constant).intValue());
	      } else if (fieldType.isLong()) {
	        return Long.valueOf(((Integer) constant).intValue());
	      }
	    }
	    return constant;
	  }
	...
	/**
	  ...
	  private soot.Value getBinaryLocal2(polyglot.ast.Binary binary, boolean reduceAggressively) {
	    ...
	    if ("java.lang.String".equals(binary.type().toString())) {
	...
	private String createStringConstant(polyglot.ast.Node node) {
	    ...
	    if (node instanceof polyglot.ast.StringLit) {
	      ...
	      s = Long.toString(((polyglot.ast.IntLit) node).value());
	    } else if (node instanceof polyglot.ast.FloatLit) {
	      s = Double.toString(((polyglot.ast.FloatLit) node).value());
	      ...
	      if (field.fieldInstance().constantValue() instanceof String) {
	        ...
	        if (num instanceof Long) {
	          s = Long.toString(((Long) num).longValue());
	        } else if (num instanceof Double) {
	          s = Double.toString(((Double) num).doubleValue());
	        } else if (num instanceof Float) {
	          s = Float.toString(((Float) num).floatValue());
	        } else if (num instanceof Byte) {
	          s = "" + ((Byte) num).byteValue();
	        } else if (num instanceof Short) {
	          s = "" + ((Short) num).shortValue();
	        } else {
	          s = Integer.toString(((Integer) num).intValue());
	        }
	      }
	...
	private String createStringConstantBinary(polyglot.ast.Binary binary) {
	    ...
	    String s = null;
	    if ("java.lang.String".equals(Util.getSootType(binary.type()).toString())) {
	...
	private boolean isStringConcat(polyglot.ast.Expr expr) {
	    if (expr instanceof polyglot.ast.Binary) {
	      ...
	      if (bin.operator() == polyglot.ast.Binary.ADD) {
	        if ("java.lang.String".equals(bin.type().toString())) {
	          return true;
	        }
	        return false;
	      }
	      ...
	      if (assign.operator() == polyglot.ast.Assign.ADD_ASSIGN) {
	        if ("java.lang.String".equals(assign.type().toString())) {
	          return true;
	        }
	        return false;
	      }
	    ...
	    if (isStringConcat(expr)) {
	      ...
	      if (toApp instanceof soot.jimple.StringConstant) {
	        ...
	        if (((soot.Local) toApp).getType() instanceof soot.PrimType) {
	          ...
	          if ("java.lang.String".equals(((soot.Local) toApp).getType().toString())) {
	            appendType = soot.RefType.v("java.lang.String");
	          } else if ("java.lang.StringBuffer".equals(((soot.Local) toApp).getType().toString())) {
	...
	/**
	  ...
	  private soot.Value getCastLocal(polyglot.ast.Cast castExpr) {
	    ...
	    if (castExpr.expr().type().equals(castExpr.type())
	        || (castExpr.type().isClass() && "java.lang.Object".equals(Util.getSootType(castExpr.type()).toString()))) {
	...
	/**
	  ...
	  private void handleFinalLocalParams(ArrayList sootParams, ArrayList sootParamTypes, polyglot.types.ClassType keyType) {
	    ...
	    boolean condition = finalLocalInfo != null && finalLocalInfo.containsKey(new polyglot.util.IdentityKey(keyType));
		if (condition) {
	        ...
	        if (finalLocals != null) {
	          finalLocals.stream().map(next -> (polyglot.types.LocalInstance) ((polyglot.util.IdentityKey) next).object()).forEach(li -> {
	...
	/**
	  ...
	  private void createConstructorCall(polyglot.ast.ConstructorCall cCall) {
		...
		if (!("<init>".equals(body.getMethod().getName()) && (cCall.kind() == polyglot.ast.ConstructorCall.SUPER))) {
			return;
		}
		handleOuterClassThisInit();
	...
	private soot.Local getSpecialSuperQualifierLocal(polyglot.ast.Expr expr) {
	    ...
	    if (expr instanceof polyglot.ast.Call) {
	      ...
	      throw new RuntimeException(new StringBuilder().append("Trying to create special super qualifier for: ").append(expr).append(" which is not a field or call").toString());
	    }
