...
public class CFGViewer extends BodyTransformer {
  ...
  private static final String phaseFullname = new StringBuilder().append(packToJoin).append('.').append(phaseSubname).toString();
...
protected void internalTransform(Body b, String phaseName, Map<String, String> options) {
    ...
    if (!((methodsToPrint == null) || (meth.getDeclaringClass().getName().equals(methodsToPrint.get(meth.getName()))))) {
		return;
	}
  ...
  public static void main(String[] args) {
    ...
    printTransform.setDeclaredOptions(new StringBuilder().append("enabled ").append(altClassPathOptionName).append(' ').append(graphTypeOptionName).append(' ').append(irOptionName).append(' ')
			.append(multipageOptionName).append(' ').append(briefLabelOptionName).append(' ').toString());
    printTransform.setDefaultOptions(
        new StringBuilder().append("enabled ").append(altClassPathOptionName).append(": ").append(graphTypeOptionName).append(':').append(defaultGraph)
				.append(' ').append(irOptionName).append(':').append(defaultIR).append(' ').append(multipageOptionName).append(":false ")
				.append(' ').append(briefLabelOptionName).append(":false ").toString());
  ...
  private static void usage() {
    logger.info(new StringBuilder().append("Usage:\n" //
).append("   java soot.util.CFGViewer [soot options] [CFGViewer options] [class[:method]]...\n\n").append("   CFGViewer options:\n" //
).append("      (When specifying the value for an '=' option, you only\n" //
).append("       need to type enough characters to specify the choice\n" //
).append("       unambiguously, and case is ignored.)\n").append("\n" //
).append("       --alt-classpath PATH :\n" //
)
			.append("                specifies the classpath from which to load classes\n" //
).append("                that implement graph types whose names begin with 'Alt'.\n" //
).append("       --graph={").append(CFGGraphType.help(0, 70, "                ".length())).append("} :\n" //
).append("                show the specified type of graph.\n").append("                Defaults to ").append(defaultGraph)
			.append(".\n" //
).append("       --ir={").append(CFGIntermediateRep.help(0, 70, "                ".length())).append("} :\n" //
).append("                create the CFG from the specified intermediate\n" //
).append("                representation. Defaults to ").append(defaultIR).append(".\n" //
)
			.append("       --brief :\n").append("                label nodes with the unit or block index,\n" //
).append("                instead of the text of their statements.\n" //
).append("       --multipages :\n").append("                produce dot file output for multiple 8.5x11\" pages.\n" //
).append("                By default, a single page is produced.\n" //
).append("       --help :\n").append("                print this message.\n" //
).append("\n" //
)
			.append("   Particularly relevant soot options (see \"soot --help\" for details):\n" //
).append("       --soot-class-path PATH\n" //
).append("       --show-exception-dests\n" //
).append("       --throw-analysis {pedantic|unit}\n" //
).append("       --omit-excepting-unit-edges\n" //
).append("       --trim-cfgs\n").toString());
  }
  ...
  private String[] parse_options(String[] args) {
    ...
    for (int i = 0, n = args.length; i < n; i++) {
      if ("--alt-classpath".equals(args[i]) || "--alt-class-path".equals(args[i])) {
        sootArgs.add("-p");
        sootArgs.add(phaseFullname);
        sootArgs.add(new StringBuilder().append(altClassPathOptionName).append(':').append(args[++i]).toString());
      } else if (args[i].startsWith("--graph=")) {
        sootArgs.add("-p");
        sootArgs.add(phaseFullname);
        sootArgs.add(new StringBuilder().append(graphTypeOptionName).append(':').append(args[i].substring("--graph=".length())).toString());
      } else if (args[i].startsWith("--ir=")) {
        sootArgs.add("-p");
        sootArgs.add(phaseFullname);
        sootArgs.add(new StringBuilder().append(irOptionName).append(':').append(args[i].substring("--ir=".length())).toString());
      } else if ("--brief".equals(args[i])) {
        sootArgs.add("-p");
        sootArgs.add(phaseFullname);
        sootArgs.add(briefLabelOptionName + ":true");
      } else if ("--multipages".equals(args[i])) {
        sootArgs.add("-p");
        sootArgs.add(phaseFullname);
        ...
        // and exiting.
      } else if ("--soot-class-path".equals(args[i]) || "-soot-class-path".equals(args[i])
          || "--soot-classpath".equals(args[i]) || "-soot-classpath".equals(args[i]) || "--process-dir".equals(args[i])
          || "-process-dir".equals(args[i]) || "--android-jars".equals(args[i]) || "-android-jars".equals(args[i])
          || "--force-android-jar".equals(args[i]) || "-force-android-jar".equals(args[i])) {
        // Pass classpaths without treating ":" as a method specifier.
        sootArgs.add(args[i]);
        sootArgs.add(args[++i]);
      } else if ("-p".equals(args[i]) || "--phase-option".equals(args[i]) || "-phase-option".equals(args[i])) {
  ...
  protected void print_cfg(Body body) {
    ...
    filename = new StringBuilder().append(filename).append(classname).append(" ").append(methodname.replace(java.io.File.separatorChar, '.')).append(DotGraph.DOT_EXTENSION).toString();
