...
public void nameCheck() {
    if(extractSingleType(lookupType(name())) != this) {
		error(new StringBuilder().append("*** Semantic Error: type variable ").append(name()).append(" is multiply declared").toString());
	}
  }
/**
   * @ast method 
...
public void typeCheck() {
    if(!getTypeBound(0).type().isTypeVariable() && !getTypeBound(0).type().isClassDecl() && !getTypeBound(0).type().isInterfaceDecl()) {
      error(new StringBuilder().append("the first type bound must be either a type variable, or a class or interface type which ").append(getTypeBound(0).type().fullName()).append(" is not").toString());
    }
    for(int i = 1; i < getNumTypeBound(); i++) {
      if(!getTypeBound(i).type().isInterfaceDecl()) {
        error(new StringBuilder().append("type bound ").append(i).append(" must be an interface type which ").append(getTypeBound(i).type().fullName()).append(" is not").toString());
      }
    }
    ...
    for(int i = 0; i < getNumTypeBound(); i++) {
      ...
      if(typeSet.contains(erasure)) {
        if(type != erasure) {
          error(new StringBuilder().append("the erasure ").append(erasure.fullName()).append(" of typebound ").append(getTypeBound(i)).append(" is multiply declared in ").append(this).toString());
        }
        else {
        ...
        for(int j = i+1; j < getNumTypeBound(); j++) {
          ...
          for(Iterator destIter = destType.memberMethods(m.name()).iterator(); destIter.hasNext(); ) {
            ...
            if(m.sameSignature(n) && m.type() != n.type()) {
              error(new StringBuilder().append("the two bounds, ").append(type.name()).append(" and ").append(destType.name()).append(", in type variable ").append(name())
					.append(" have a method ").append(m.signature()).append(" with conflicting return types ").append(m.type().name()).append(" and ").append(n.type().name()).toString());
            }
          }
        }
      }
...
/**
  ...
  private String fullName_compute() {
	...
	return new StringBuilder().append(typeDecl.fullName()).append("@").append(name()).toString();
  }
/**
   * @attribute syn
   * @aspect LookupParTypeDecl
...
@SuppressWarnings({"unchecked", "cast"})
  ...
  int num = state.boundariesCrossed;
  boolean isFinal = this.is$Final();
    boolean accessibleFrom_TypeDecl_value = accessibleFrom_compute();
      if(isFinal && num == state().boundariesCrossed) {
...
/**
   * @apilevel internal
   */
  private boolean accessibleFrom_compute() {  return true;  }
/**
   * @attribute syn
   * @aspect NewGenerics
