...
public class CastExpr extends Expr {
  private static final Logger logger = LoggerFactory.getLogger(CastExpr.class);
/**
   * @apilevel internal
   */
...
public void typeCheck() {
	...
	if(!expr.castingConversionTo(type)) {
		error(new StringBuilder().append(expr.typeName()).append(" can not be cast into ").append(type.typeName()).toString());
	}
	if(!getTypeAccess().isTypeAccess()) {
		error(new StringBuilder().append("").append(getTypeAccess()).append(" is not a type access in cast expression").toString());
	}
  }
/**
   * @ast method 
