 ...
 */
public class InterfaceDecl extends ReferenceType {
  private static final Logger logger = LoggerFactory.getLogger(InterfaceDecl.class);
/**
   * @ast method 
...
public void accessControl() {
	...
	for(int i = 0; i < getNumSuperInterfaceId(); i++) {
        ...
        if(!decl.isInterfaceDecl() && !decl.isUnknown()) {
			error(new StringBuilder().append("interface ").append(fullName()).append(" tries to extend non interface type ").append(decl.fullName()).toString());
		}
        if(!decl.isCircular() && !decl.accessibleFrom(this)) {
			error(new StringBuilder().append("interface ").append(fullName()).append(" can not extend non accessible type ").append(decl.fullName()).toString());
		}

        if(set.contains(decl)) {
			error(new StringBuilder().append("extended interface ").append(decl.fullName()).append(" mentionened multiple times in extends clause").toString());
		}
        set.add(decl);
      }
  }
