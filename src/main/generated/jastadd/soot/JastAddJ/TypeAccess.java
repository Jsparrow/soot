...
public class TypeAccess extends Access {
  private static final Logger logger = LoggerFactory.getLogger(TypeAccess.class);
/**
   * @apilevel internal
   * @ast method 
   * 
...
public void accessControl() {
    ...
    if(hostType != null && !hostType.isUnknown() && !type().accessibleFrom(hostType)) {
      error(new StringBuilder().append("").append(this).append(" in ").append(hostType().fullName()).append(" can not access type ").append(type().fullName()).toString());
    }
    else if((hostType == null || hostType.isUnknown())  && !type().accessibleFromPackage(hostPackage())) {
      error(new StringBuilder().append("").append(this).append(" can not access type ").append(type().fullName()).toString());
    }
  }
/**
   * @ast method 
...
public void nameCheck() {
    if(isQualified() && !qualifier().isTypeAccess() && !qualifier().isPackageAccess()) {
		error(new StringBuilder().append("can not access the type named ").append(decl().typeName()).append(" in this context").toString());
	}
    if(decls().isEmpty()) {
	...
	s.append(new StringBuilder().append("several types named ").append(name()).append(":").toString());
...
public String dumpString() {
    ASTNode$State state = state();
    try {  return new StringBuilder().append(getClass().getName()).append(" [").append(getPackage()).append(", ").append(getID()).append("]")
			.toString();  }
    finally {
    }
  }
...
/**
  ...
  public String nameWithPackage() {
    ASTNode$State state = state();
    try {  return "".equals(getPackage()) ? name() : (new StringBuilder().append(getPackage()).append(".").append(name()).toString());  }
    finally {
    }
  }
/**
...
public String typeName() {
    ASTNode$State state = state();
    try {  return isQualified() ? (new StringBuilder().append(qualifier().typeName()).append(".").append(name()).toString()) : nameWithPackage();  }
    finally {
    }
  }
/**
