 ...
 */
public class GenericClassDecl extends ClassDecl implements GenericTypeDecl {
  private static final Logger logger = LoggerFactory.getLogger(GenericClassDecl.class);
/**
   * @apilevel internal
   */
...
public void typeCheck() {
    ...
    if(instanceOf(typeThrowable())) {
		error(new StringBuilder().append(" generic class ").append(typeName()).append(" may not directly or indirectly inherit java.lang.Throwable").toString());
	}
  }
/**
   * @ast method 
  ...
  private List<PlaceholderMethodDecl> getPlaceholderMethodList_compute() {
		...
		for (Iterator iter = constructors().iterator(); iter.hasNext(); ) {
			...
			if (decl instanceof GenericConstructorDecl) {
				...
				List<TypeVariable> typeVariables = new List<>();
