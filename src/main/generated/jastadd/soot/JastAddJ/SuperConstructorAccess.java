 ...
 */
public class SuperConstructorAccess extends ConstructorAccess {
  private static final Logger logger = LoggerFactory.getLogger(SuperConstructorAccess.class);
/**
   * @apilevel internal
   */
...
public void nameCheck() {
    ...
    if(isQualified()) {
      if(!s.isInnerType() || s.inStaticContext()) {
		error(new StringBuilder().append("the super type ").append(s.typeName()).append(" of ").append(c.typeName()).append(" is not an inner class").toString());
	} else if(!qualifier().type().instanceOf(s.enclosingType())) {
		error(new StringBuilder().append("The type of this primary expression, ").append(qualifier().type().typeName()).append(" is not enclosing the super type, ").append(s.typeName()).append(", of ").append(c.typeName())
				.toString());
	}
    }
    boolean condition = !isQualified() && s.isInnerType() && !c.isInnerType();
	if(condition) {
        error(new StringBuilder().append("no enclosing instance for ").append(s.typeName()).append(" when accessed in ").append(this).toString());
      }
