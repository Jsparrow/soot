...
public class ReturnStmt extends Stmt {
  private static final Logger logger = LoggerFactory.getLogger(ReturnStmt.class);
/**
   * @apilevel internal
   */
...
public void typeCheck() {
    boolean condition = hasResult() && !returnType().isVoid() && !getResult().type().assignConversionTo(returnType(), getResult());
	if(condition) {
		error(new StringBuilder().append("return value must be an instance of ").append(returnType().typeName()).append(" which ").append(getResult().type().typeName()).append(" is not").toString());
	}
    // 8.4.5 8.8.5
