...
public void checkModifiers() {
	...
	if(m != null && m.getNumElementValuePair() == 1 && "value".equals(m.getElementValuePair(0).getName())) {
        ElementValue v = m.getElementValuePair(0).getElementValue();
        //System.out.println("ElementValue: \n" + v.dumpTree());
        //System.out.println("Annotation: \n" + dumpTree());
        if(!v.validTarget(this)) {
			error(new StringBuilder().append("annotation type ").append(T.typeName()).append(" is not applicable to this kind of declaration").toString());
		}
      }
  }
/**
    ...
    if(!decl().isAnnotationDecl()) {
      ...
      for(int i = 0; i < getNumElementValuePair(); i++) {
        ...
        if(typeDecl.memberMethods(pair.getName()).isEmpty()) {
			error(new StringBuilder().append("can not find element named ").append(pair.getName()).append(" in ").append(typeDecl.typeName()).toString());
		}
      }
    }
    checkOverride();
...
/**
   ...
   */
	...
	public void checkOverride() {
		if (!("java.lang.Override".equals(decl().fullName()) &&
...
public boolean isRuntimeVisible() {
    ...
    return v != null && "RUNTIME".equals(v.name());
  }
    finally {
    }
  }
...
public boolean isRuntimeInvisible() {
    ...
    if(a == null)
	 {
		return true; // default bahavior if not annotated
	}
    ElementConstantValue value = (ElementConstantValue)a.getElementValuePair(0).getElementValue();
    Variable v = value.getExpr().varDecl();
    return v != null &&  "CLASS".equals(v.name());
  }
    finally {
    }
  }
