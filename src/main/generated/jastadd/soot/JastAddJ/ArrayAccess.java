...
public class ArrayAccess extends Access {
  private static final Logger logger = LoggerFactory.getLogger(ArrayAccess.class);
/**
   * @apilevel internal
   */
...
public void typeCheck() {
    if(isQualified() && !qualifier().type().isArrayDecl() && !qualifier().type().isUnknown()) {
		error(new StringBuilder().append("the type ").append(qualifier().type().name()).append(" of the indexed element is not an array").toString());
	}
    if(!getExpr().type().unaryNumericPromotion().isInt() || !getExpr().type().isIntegralType()) {
		error(new StringBuilder().append("array index must be int after unary numeric promotion which ").append(getExpr().type().typeName()).append(" is not").toString());
	}
  }
/**
   * @ast method 
