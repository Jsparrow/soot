 ...
 */
public class Program extends ASTNode<ASTNode> {
	...
	public Object next() {
        if(getNumCompilationUnit() == index) {
          ...
          if(u != null) {
			...
			throw new Error(new StringBuilder().append("File ").append(typename).append(" not found").toString());
		}
        }
...
/**
  ...
  public void pushClassPath(String name) {
    ...
    if(part != null) {
      sourcePath.add(part);
      logger.info("Pushing source path " + name);
    } else {
	...
	classPath.add(part);
	logger.info("Pushing class path " + name);
  }
/**
   * @ast method 
   * @aspect ClassPath
...
/**
  ...
  public void initPaths() {
	...
	if(!options().hasValueForOption("-sourcepath")) {
        for (String userClasse : userClasses) {
          classPaths.add(userClasse);
          sourcePaths.add(userClasse);
	...
	for(Iterator iter = classPaths.iterator(); iter.hasNext(); ) {
        ...
        if(part != null) {
          ...
          //System.out.println("Adding classpath " + s);
			...
			logger.info(new StringBuilder().append("Warning: Could not use ").append(s).append(" as class path").toString());
		}
      }
	for(Iterator iter = sourcePaths.iterator(); iter.hasNext(); ) {
        ...
        if(part != null) {
          ...
          //System.out.println("Adding sourcepath " + s);
			...
			logger.info(new StringBuilder().append("Warning: Could not use ").append(s).append(" as source path").toString());
		}
      }
  }
/**
...
/**
  ...
  public boolean hasPackage(String packageName) {
    ...
    if(hasPackage_String_values == null) {
		hasPackage_String_values = new java.util.HashMap(4);
	}
...
/**
  ...
  private TypeDecl lookupType_compute(String packageName, String typeName) {
    String fullName = "".equals(packageName) ? typeName : new StringBuilder().append(packageName).append(".").append(typeName).toString();
...
/**
  ...
  private TypeDecl lookupLibType_compute(String packageName, String typeName) {
    ...
    if (packageName.equals(PRIMITIVE_PACKAGE_NAME)) {
	 ...
	 {
		return unit.unknownType(); // Is this needed?
	}
    } 
    // Check the library:
