 ...
 */
public class AnnotationMethodDecl extends MethodDecl {
  private static final Logger logger = LoggerFactory.getLogger(AnnotationMethodDecl.class);
/**
   * @ast method 
   * 
   */
...
public void typeCheck() {
    ...
    if(hasDefaultValue() && !type().commensurateWith(getDefaultValue())) {
		error(new StringBuilder().append(type().typeName()).append(" is not commensurate with ").append(getDefaultValue().type().typeName()).toString());
	}
  }
/**
   * @ast method 
