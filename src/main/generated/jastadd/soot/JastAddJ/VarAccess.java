...
public class VarAccess extends Access {
  private static final Logger logger = LoggerFactory.getLogger(VarAccess.class);
/**
   * @apilevel internal
   * @ast method 
   * 
...
public void definiteAssignment() {
    if(isSource()) {
      if(decl() instanceof VariableDeclaration) {
        ...
        if(v.isValue()) {
          ...
          if(!isDAbefore(v)) {
			error(new StringBuilder().append("Final variable ").append(v.name()).append(" is not assigned before used").toString());
		}
        }
        else {
          //if(!v.hasInit() && !isDAbefore(v)) {
          if(!isDAbefore(v)) {
			error(new StringBuilder().append("Local variable ").append(v.name()).append(" is not assigned before used").toString());
		}
        }
      }
        ...
        if(f.isFinal() && !f.hasInit() && !isDAbefore(f)) {
          error(new StringBuilder().append("Final field ").append(f).append(" is not assigned before used").toString());
        }
      }
	...
	// Blank final field
      if(v.isFinal() && v.isBlank() && !hostType().instanceOf(v.hostType())) {
		...
		error(new StringBuilder().append("the blank final field ").append(v.name()).append(" may only be assigned by simple name").toString());
        ...
        if(!var.isValue() && var.getParent().getParent().getParent() instanceof SwitchStmt && var.isFinal()) {
          if(!isDUbefore(var)) {
			error(new StringBuilder().append("Final variable ").append(var.name()).append(" may only be assigned once").toString());
		}
        }
        else if(var.isValue()) {
      ...
      // field
        ...
        if(f.isFinal()) {
          if(f.hasInit()) {
			error(new StringBuilder().append("initialized field ").append(f.name()).append(" can not be assigned").toString());
		} else {
            ...
            if(!(bodyDecl instanceof ConstructorDecl) && !(bodyDecl instanceof InstanceInitializer) && !(bodyDecl instanceof StaticInitializer) && !(bodyDecl instanceof FieldDeclaration)) {
				error(new StringBuilder().append("final field ").append(f.name()).append(" may only be assigned in constructors and initializers").toString());
			} else if(!isDUbefore(f)) {
				error(new StringBuilder().append("Final field ").append(f.name()).append(" may only be assigned once").toString());
			}
          }
        }
      }
      else if(v.isParameter() && v.isFinal()) {
	  error(new StringBuilder().append("Final parameter ").append(v.name()).append(" may not be assigned").toString());
	}
  }
/**
   * @ast method 
...
public void nameCheck() {
    if(decls().isEmpty() && (!isQualified() || !qualifier().type().isUnknown() || qualifier().isPackageAccess())) {
		error(new StringBuilder().append("no field named ").append(name()).append(" is accessible").toString());
	}
    if(decls().size() > 1) {
      ...
      for(Iterator iter = decls().iterator(); iter.hasNext(); ) {
        ...
        s.append(new StringBuilder().append("\n    ").append(v.type().typeName()).append(".").append(v.name()).append(" declared in ").append(v.hostType().typeName())
				.toString());
      }
      error(s.toString());
    }
    ...
    if(inExplicitConstructorInvocation() && !isQualified() && decl().isInstanceVariable() && hostType() == decl().hostType()) {
		error(new StringBuilder().append("instance variable ").append(name()).append(" may not be accessed in an explicit constructor invocation").toString());
	}
    ...
    boolean condition = (decl().isInstanceVariable() || decl().isClassVariable()) && !isQualified() && hostType() != null && !hostType().declaredBeforeUse(decl(), this) && inSameInitializer() && !simpleAssignment() && inDeclaringClass();
	// 8.3.2.3
    if(condition) {
	  BodyDecl b = closestBodyDecl(hostType());
	  error(new StringBuilder().append("variable ").append(decl().name()).append(" is used in ").append(b).append(" before it is declared").toString());
	}
...
public void checkModifiers() {
	...
	if(f.isDeprecated() &&
		...
		warning(new StringBuilder().append(f.name()).append(" in ").append(f.hostType().typeName()).append(" has been deprecated").toString());
	}
  }
/**
   * @ast method 
...
/**
  ...
  public soot.Value refined_Expressions_VarAccess_eval(Body b) {
    ...
    if(v instanceof VariableDeclaration) {
      ...
      FieldDeclaration f = (FieldDeclaration)v;
      if(f.hostType().isArrayDecl() && "length".equals(f.name())) {
...
/**
  ...
  public soot.Local base(Body b) {
    ...
    if(base_Body_values == null) {
		base_Body_values = new java.util.HashMap(4);
	}
