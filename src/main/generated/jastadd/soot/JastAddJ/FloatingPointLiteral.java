 ...
 */
public class FloatingPointLiteral extends NumericLiteral {
  private static final Logger logger = LoggerFactory.getLogger(FloatingPointLiteral.class);
/**
   * @apilevel internal
   */
...
public void typeCheck() {
   if(!isZero() && constant().floatValue() == 0.0f) {
	error(new StringBuilder().append("It is an error for nonzero floating-point ").append(getLITERAL()).append(" to round to zero").toString());
}
   if(constant().floatValue() == Float.NEGATIVE_INFINITY || constant().floatValue() == Float.POSITIVE_INFINITY) {
	error(new StringBuilder().append("It is an error for floating-point ").append(getLITERAL()).append(" to round to an infinity").toString());
}
