 ...
 * 
 */
public class ASTNode<T extends ASTNode> extends beaver.Symbol  implements Iterable<T> {
...
/**
  ...
  public String errorPrefix() {
    return new StringBuilder().append(sourceFile()).append(":").append(location()).append(":\n").append("  *** Semantic Error: ").toString();
  }
/**
   * @ast method 
   * @aspect ErrorCheck
...
/**
  ...
  public soot.Value asRValue(Body b, soot.Value v) {
    ...
    throw new Error(new StringBuilder().append("Need to convert ").append(v.getClass().getName()).append(" to RValue").toString());
  }
/**
   * @ast method 
   * @aspect EmitJimple
...
/**
  ...
  public void checkUncheckedConversion(TypeDecl source, TypeDecl dest) {
    if (source.isUncheckedConversionTo(dest)) {
		warning(new StringBuilder().append("unchecked conversion from raw type ").append(source.typeName()).append(" to generic type ").append(dest.typeName()).toString());
	}
  }
/**
