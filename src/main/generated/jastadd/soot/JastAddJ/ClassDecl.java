 ...
 */
public class ClassDecl extends ReferenceType {
  private static final Logger logger = LoggerFactory.getLogger(ClassDecl.class);
/**
   * @apilevel internal
   */
...
public void accessControl() {
    ...
    if(typeDecl != null && !typeDecl.accessibleFromExtend(this)) {
		//if(typeDecl != null && !isCircular() && !typeDecl.accessibleFrom(this))
		  error(new StringBuilder().append("class ").append(fullName()).append(" may not extend non accessible type ").append(typeDecl.fullName()).toString());
	}

    if(hasSuperclass() && !superclass().accessibleFrom(this)) {
		error(new StringBuilder().append("a superclass must be accessible which ").append(superclass().name()).append(" is not").toString());
	}
    ...
    for(int i = 0; i < getNumImplements(); i++) {
      ...
      if(!decl.isCircular() && !decl.accessibleFrom(this)) {
		error(new StringBuilder().append("class ").append(fullName()).append(" can not implement non accessible type ").append(decl.fullName()).toString());
	}
    }
  }
/**
...
public void checkModifiers() {
    ...
    if(typeDecl != null && typeDecl.isFinal()) {
      error(new StringBuilder().append("class ").append(fullName()).append(" may not extend final class ").append(typeDecl.fullName()).toString());
    }
...
public void nameCheck() {
    ...
    for(int i = 0; i < getNumImplements(); i++) {
      ...
      if(!decl.isInterfaceDecl() && !decl.isUnknown()) {
		error(new StringBuilder().append("type ").append(fullName()).append(" tries to implement non interface type ").append(decl.fullName()).toString());
	}
      if(set.contains(decl)) {
		error(new StringBuilder().append("type ").append(decl.fullName()).append(" mentionened multiple times in implements clause").toString());
	}
      set.add(decl);
    }
        ...
        if(s.isEmpty()) {
          for(Iterator i2 = interfacesMethodsSignature(m.signature()).iterator(); i2.hasNext(); ) {
            ...
            if(!n.mayOverrideReturn(m) && !m.mayOverrideReturn(n)) {
				error(new StringBuilder().append("Xthe return type of method ").append(m.signature()).append(" in ").append(m.hostType().typeName()).append(" does not match the return type of method ")
						.append(n.signature()).append(" in ").append(n.hostType().typeName()).append(" and may thus not be overriden").toString());
			}
          }
        }
      }
...
/**
  ...
  private void interfaceMethodCompatibleWithInherited(MethodDecl m, MethodDecl n) {
    ...
    if(!n.isAbstract() && !n.isPublic()) {
		error(new StringBuilder().append("Xoverriding access modifier error for ").append(m.signature()).append(" in ").append(m.hostType().typeName()).append(" and ").append(n.hostType().typeName())
				.toString());
	}
    if(!n.mayOverrideReturn(m) && !m.mayOverrideReturn(m)) {
		error(new StringBuilder().append("Xthe return type of method ").append(m.signature()).append(" in ").append(m.hostType().typeName()).append(" does not match the return type of method ").append(n.signature())
				.append(" in ").append(n.hostType().typeName()).append(" and may thus not be overriden").toString());
	}
    if(!n.isAbstract()) {
      ...
      for(int i = 0; i < n.getNumException(); i++) {
        ...
        if(!found && e.type().isUncheckedException()) {
			error(new StringBuilder().append("X").append(n.signature()).append(" in ").append(n.hostType().typeName()).append(" may not throw more checked exceptions than overridden method ").append(m.signature())
					.append(" in ").append(m.hostType().typeName()).toString());
		}
      }
    }
  }
  ...
  private SootClass refined_EmitJimpleRefinements_ClassDecl_sootClass()
{
    ...
    if(needAddclass) {
      if(options().verbose()) {
		logger.info("Creating from source " + jvmName());
	}        
  ...
  private HashMap methodsSignatureMap_compute() {
    ...
    if(hasSuperclass()) {
      for(Iterator iter = superclass().methodsIterator(); iter.hasNext(); ) {
        MethodDecl m = (MethodDecl)iter.next();
        boolean condition = !m.isPrivate() && m.accessibleFrom(this) && !localMethodsSignatureMap().containsKey(m.signature()) && (!(m instanceof MethodDeclSubstituted) || !localMethodsSignatureMap().containsKey(m.sourceMethodDecl().signature()));
		if(condition) {
			putSimpleSetElement(map, m.signature(), m);
		}
      }
    ...
    for(Iterator outerIter = interfacesIterator(); outerIter.hasNext(); ) {
      ...
      for(Iterator iter = typeDecl.methodsIterator(); iter.hasNext(); ) {
        MethodDecl m = (MethodDecl)iter.next();
        boolean condition1 = !m.isPrivate() && m.accessibleFrom(this) && !localMethodsSignatureMap().containsKey(m.signature()) && (!(m instanceof MethodDeclSubstituted) || !localMethodsSignatureMap().containsKey(m.sourceMethodDecl().signature())) && allMethodsAbstract((SimpleSet)map.get(m.signature())) &&
...
/**
   * @apilevel internal
   */
  private boolean isString_compute() {  return "java.lang.String".equals(fullName());  }
/**
   * @attribute syn
   * @aspect TypeAnalysis
...
/**
   * @apilevel internal
   */
  private boolean isObject_compute() {  return "Object".equals(name()) && "java.lang".equals(packageName());  }
/**
   * @attribute syn
...
public Annotation annotation(TypeDecl typeDecl) {
    ...
    boolean condition = hasSuperclass() && typeDecl.annotation(lookupType("java.lang.annotation", "Inherited")) != null;
...
public String typeDescriptor() {
    ASTNode$State state = state();
    try {  return new StringBuilder().append("L").append(jvmName().replace('.', '/')).append(";").toString();  }
    finally {
    }
  }
/**
