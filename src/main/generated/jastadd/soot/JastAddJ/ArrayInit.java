...
public class ArrayInit extends Expr {
  private static final Logger logger = LoggerFactory.getLogger(ArrayInit.class);
...
public void typeCheck() {
    ...
    for(int i = 0; i < getNumInit(); i++) {
      ...
      if(!e.type().assignConversionTo(initializerType, e)) {
		error(new StringBuilder().append("the type ").append(e.type().name()).append(" of the initializer is not compatible with ").append(initializerType.name()).toString());
	} 
    }
  }
/**
